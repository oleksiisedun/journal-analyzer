...
- `C` (`REPORT_COLUMN`) — output; empty until processed

Every row is scanned using the single hardcoded `POSITION_HEADER_REGEX` (`Config.js`) — there is no per-row regex column anymore.

Running the "Journal Analyzer → Run analysis" menu item processes exactly **one** row per invocation: the first row (top to bottom) with non-empty B and empty C. Multiple rows require multiple menu invocations.
...
- **Column positions are resolved at runtime, not hardcoded**: `Config.js` defines `FOLDER_LINK_COLUMN` / `REPORT_COLUMN` as open-ended A1 ranges (e.g. `'C2:C'`), and `SheetUtils.columnNumber_` converts one to a numeric index via `sheet.getRange(a1).getColumn()`.
- **Invalid or empty outcomes are written back to the report column**, never left blank, so a row is never silently reprocessed or silently skipped forever: `ERROR_INVALID_LINK` for rows that fail validation, `NO_FILES_MARKER` / `NO_MATCH_MARKER` for folders that produce nothing. Files that fail to process (undated filename, conversion error) are collected into `skippedFiles` and shown inline in `Progress.html` once the run finishes.
